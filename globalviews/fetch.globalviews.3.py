...
class Product():
    ...
    def __init__(self):
        ...
        self.brand_name = "Global Views"
        self.brand_alia = "globalviews"
        self.base_url = "https://www.globalviews.com/categories"
    ...
    def get_product_tag(self, product_url, product_html):
        ...
        # sku & production_name & size & description
        tag_table = product_html.find("table", id="detailstable")
        tag_tr_1 = tag_table.find_all("tr")[1]
        tag_td = tag_tr_1.find_all("td")
        sku = tag_td[0].get_text().strip()
        print("sku:", sku)
        production_name = tag_td[1].get_text().strip()
        print("production_name:", production_name)
        size = ""
        for tag_li in tag_td[2].find_all("li"):
            size += tag_li.get_text().strip() + ","
        size = size[:-1]
        print("size:", size)
        
        tag_tr_2 = tag_table.find_all("tr")[-1]
        description = tag_tr_2.find("span", class_="product_description").get_text().strip()
    ...
    def get_product_img_urls(self, product_html):
        img_urls = []
        tag_li = product_html.find("ul", class_="thumblist").find_all("li")
        for li in tag_li:
            img_url = li.find("a").get("href")
    ...
    def get_product_urls(self, html):
        ...
        # 先处理类别
        for li in html.find_all('li', class_='productcategoryname'):            
            categorie_url = "https://www.globalviews.com" + li.find('a').get('href') + "?all=true"
            categorie_name = li.find('a').get_text()
            ...
            for li in categorie_html.find("ul", id="productlist").find_all("li"):
                product_url = "https://www.globalviews.com" + li.find("a").get("href")
    ...
    def start(self):
        ...
        if brand_products:
            ...
            self.brand_collection.save({"brand_alia": self.brand_alia,"brand_product_urls": product_urls,"created_time": datetime.datetime.now()})
        ...
        for product_url in product_urls:
            ...
            if self.product_collection.find_one({"product_url": product_url}):
                ...
                if num < 3001:
                    ...
                    continue
                if num > 4000:
                    print(u"3001-4000数据获取完成")
                    return
                print("(" + str(num) + "/" + str(len(product_urls)) + ")", sep="")
                if self.proxyOn and num%3==0:
                    self.proxy = self.get_proxy()
